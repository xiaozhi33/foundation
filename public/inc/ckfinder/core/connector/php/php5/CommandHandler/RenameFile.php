...
class CKFinder_Connector_CommandHandler_RenameFile extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    public function buildXml()
    {
		...
		//判断文件命名规则(字母数字下划线中划线，后缀小写)
		$tempFileNameInfo = pathinfo($_GET["newFileName"]);
		if(preg_match('/[^-a-z0-9_A-Z]/',$tempFileNameInfo['filename'])) {
			$this->_errorHandler->throwError(CKFINDER_CONNECTOR_ERROR_INVALID_NAME);
		}
		$_GET["newFileName"] = $tempFileNameInfo['filename'] . '.' . strtolower($tempFileNameInfo['extension']);

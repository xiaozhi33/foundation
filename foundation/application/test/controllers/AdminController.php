<?php
	...
	class Test_adminController extends BaseController {
		private $dbhelper;
            ...
            try{
                if($_REQUEST['name'] != "" ){
                    ...
                    if($rs){
                        ...
                        alert_go("添加成功！", "/test/admin/department");
                    }else {
		...
		public function editrsdepartmentAction(){
			if($_REQUEST['name'] != "" && $_REQUEST['id']){
				...
				alert_go("部门编辑成功。","/test/admin/department");
			}else {
				alert_back("添加失败");
			}
		}
		...
		/*public function deldepartmentAction(){
			...
			if(!empty($id)){
				...
				echo "window.location.href='/test/admin/department'";
				echo "</script>";
				exit();
			}
		}*/
		...
		public function addrscateAction(){
			if($_REQUEST['name'] != ""){
				...
				alert_go("项目分类添加成功。","/test/admin/cate");
			}else {
				alert_back("添加失败");
			}
		}
		...
		public function editrscateAction(){
			if($_REQUEST['name'] != "" && $_REQUEST['id']){
				...
				$cateinfo ->catename = $_REQUEST['name'];
				$cateinfo->save($this->dbhelper);
				alert_go("部门编辑成功。","/test/admin/cate");
			}else {
				alert_back("添加失败");
			}
		}
		...
		public function addrsppAction(){
			if($_REQUEST['ppname'] != ""){
				...
				// alert_go("联系人添加成功。","/test/admin/pp?pp_cate=".$_REQUEST['pp_cate']);
                alert_go("联系人添加成功。","/test/admin/pp");
			}else {
				alert_back("添加失败");
			}
		}
		...
		public function editrsppAction(){
			if($_REQUEST['ppname'] != "" && $_REQUEST['pid']){
				...
				$ppinfo ->save($this->dbhelper);
				// alert_go("编辑成功。","/test/admin/pp?pp_cate=".$_REQUEST['pp_cate']);
                alert_go("编辑成功。","/test/admin/pp");
			}else {
				alert_back("添加失败");
			}
		}
		...
		public function editrspwdAction(){
			...
			if($name !="" && $pwd != ""){
				...
				alert_go("密码修改成功","/test/admin/editpwd");
			}else{
        ...
        public function delppcompanyAction()
        {
            ...
            if(empty($id)) {
                alert_back("操作失败。");
                $this->_redirect("/test/admin/ppcompany?pp_id=".$pp_id);
            }
            $jjh_mg_pp_companyDAO = $this->orm->createDAO("jjh_mg_pp_company");
            $jjh_mg_pp_companyDAO->findid($id);
		...
		public function delpphuodongAction()
		{
			...
			if(empty($id)) {
				alert_back("操作失败。");
				$this->_redirect("/test/admin/pphuodong?pp_id=".$pp_id);
			}
			$pm_mg_info_activeDAO = $this->orm->createDAO("pm_mg_info_active");
			$pm_mg_info_activeDAO->findid($id);
		...
		public function delppvisitAction()
		{
			...
			if(empty($id)) {
				alert_back("操作失败。");
				$this->_redirect("/test/admin/ppvisit?pp_id=".$pp_id);
			}
			$pm_mg_visit_donorDAO = $this->orm->createDAO("pm_mg_visit_donor");
			$pm_mg_visit_donorDAO->findid($id);
        ...
        public function syncjzfAction(){
            ...
            */
            ...
            echo('<script language="JavaScript">');
            echo("alert('同步已完成');");
            echo("location.href='/test/admin/pp?pp_cate=捐赠方';");
            echo('</script>');
            exit;
        }

...
class Test_taskController extends BaseController
{
    private $dbhelper;
    ...
    public function indexAction()
    {
        ...
        $taskDAO->getPager(array('path'=>'/test/task/index'))->assignTo($this->view);
    ...
    public function toAddtaskmainAction(){
        ...
        if(empty($id)){
            ...
            echo('<script language="JavaScript">');
            echo("alert('保存成功');");
            echo("location.href='/test/task';");
            echo('</script>');
            exit;
        }else {
            echo json_encode(array('msg'=>"保存成功！",'return_url'=>'/test/task/'));
            exit;
        }
    }

<?php
	...
	class Test_MeetingController extends BaseController
    {

		public function indexAction(){
            //$this->_redirect('/admin/editor/spec-list');
            $meetingDAO = $this->orm->createDAO('jjh_meeting')->order('id DESC');
            $meetingDAO->getPager(array('path'=>'/test/meeeting/index'))->assignTo($this->view);
        ...
        public function toAddAction(){
            ...
            if($_FILES['meeting_files']['name']!=""){
                if($_FILES['meeting_files']['error'] != 4){
                    ...
                    if($result['error']!=0){
                        ...
                        echo "window.location.href='/test/meeting';";
                        echo "</script>";
                        exit();
                    }else{
            ...
            echo('<script language="JavaScript">');
            echo("alert('保存成功');");
            echo("location.href='/test/meeting';");
            echo('</script>');
            exit;

            /*echo json_encode(array('msg'=>"保存成功！",'return_url'=>'/test/meeting/'));
            exit;*/
        }

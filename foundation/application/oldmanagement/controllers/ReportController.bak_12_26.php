	...
	class Management_reportController extends BaseController {
		...
		public function reportinfoAction(){
			...
			if($_REQUEST['style'] == "style1"){
				...
				if($start != "" && $end != "" || $pname != ""){
					$selectSQL .= "select sum(zijin_daozheng_jiner) as daozhang,sum(shiyong_zhichu_jiner) as shiyong,pm_name ";
					$selectSQL .= "from pm_mg_info where";
					
					if ($start != "" && $end != ""){
						//$selectSQL .= " zijin_daozhang_datetime>='$start' or shiyong_zhichu_datetime<='$end' ";
						$selectSQL .= " zijin_daozhang_datetime between '$start' and '$end' or shiyong_zhichu_datetime between '$start' and '$end' ";
					}

					if($start != "" && $end != "" && $pname != ""){
						$selectSQL .= " and pm_name = '$pname'";
					}
					
					if($start == "" && $end == "" && $pname != ""){
						$selectSQL .= " pm_name = '$pname'";
					}
					
					$selectSQL .= ' group by pm_name';

            ...
            if(!empty($zw_pzfl_list)){
                foreach($zw_pzfl_list as $key => $value){  // 批量添加财务来款到项目info中
					...
					if(!empty($zw_pm_relatedDAO[0]['pm_name'])){
						$islog = $this->getisuselog($value['pzrq'],$zw_pm_relatedDAO[0]['pm_name'],$value['jje'],$zw_pm_relatedDAO['pm_id']);

...
class uploadPic{
    ...
    function show_execute_message($smfileerror,$smfilename,$smfiletype,$smfilesize,$type){
        if($smfileerror>0){
            switch ($smfileerror){
                case 1: $smfilemessage='<strong>上传文件超过限制大小</strong>';break;
                case 2: $smfilemessage='<strong>上传文件超过限制大小</strong>';break;
                case 3: $smfilemessage='<strong>上传文件未完成</strong>';break;
            ...
            /*������ʵ��ʽ��֤
             ...
             if($smfiletypeflag == false){ //�ж��Ƿ���pngͼƬ
             $typeCode = intval($strInfo['chars2'].$strInfo['chars3'].$strInfo['chars4']);
             $smfiletypeflag = array_search($typeCode, $this->file_type_real_array);
             if($smfiletypeflag == false){ //�ж��Ƿ���jpgͼƬ
            ...
            if($smfiletypeflag == false){
                if($type == 2){
                    $smfilemessage='<strong>上传不成功,请选择flash文件上传</strong>';
                }else{
                    $smfilemessage='<strong>上传不成功,请选择图片文件上传</strong>';
                }
                ...
                if ($resflag[0] == 1){
                    ...
                    if($type == 2){
                        $smfilemessage='<strong>flash上传不成功</strong>';
                    }else{
                        $smfilemessage = '<strong>图片上传不成功</strong>';
                    }
    ...
    function size_BKM($size){ // B/KB/MB��λת��
        if($size < 1024)
        {
